...
void CriticalAwareV3::apply(uint64_t current_interval, const tasklist_t &tasklist)
{
    ...
    for (const auto &item : v_mpkil3)
    {
        ...
        else if ((MPKIL3Task >= limit_outlier) & (itX == id_isolated.end()) & (HPKIL3Task >= MPKIL3Task))
        {
        ...
        {
			// it's not a critical app
			if ((MPKIL3Task >= limit_outlier) & (HPKIL3Task < MPKIL3Task))
			{
    ...
    if (firstTime)
    {
        ...
        for (const auto &item : outlier)
        {
			...
			{
				LOGINF("Task ID {} isolated in CLOS 3"_format(idTask));
				ipc_NCR += ipcTask;
			}
        }
	}
