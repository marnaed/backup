...
std::shared_ptr<cat::policy::Base> config_read_cat_policy(const YAML::Node &config)
{
 	...
 	if (kind == "cav3")
	{
		...
		// Check that required fields exist
		for (string field : {"every", "firstInterval", "IDLE_INTERVALS", "ipc_threshold"})
		{
			if (!policy[field])
				throw_with_trace(std::runtime_error("The '" + kind + "' CAT policy needs the '" + field + "' field"));
		}
		...
		return std::make_shared<cat::policy::CriticalAwareV3>(every, firstInterval, IDLE_INTERVALS, ipc_threshold);
	}
	else if (kind == "cav2")
    {
